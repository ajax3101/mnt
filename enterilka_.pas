...
//     aaa.RichEdit1.Lines.Add('        *****   Динамика пуска конвейера  ');
//     WriteLn;
//     aaa.RichEdit1.Lines.Add('');
{ Write( '      t         y1         y2         y3');}
{   WriteLn( '        y4          F ');}
...
//     aaa.RichEdit1.Lines.Add(' ******* Второй этап расчета');
        {Write ('    ', x:2:5 );}

//        aaa.Series1.AddXY(0,0);
        aaa.Series1.AddXy(x,fff,'Второй этап',clBlue);
...
//      if n = 2 then {WriteLn ('                         ',f);}
  ...
  le: //  close (fil);
        ...
        aaa.Caption:='График студента '+fio_edit.Text
        end;
...
a:tform1;
begin
     Caption:='Исходные данные студента '+fio_edit.Text;
     try
         ...
         cr[1] := 4.8 * cc[1] * Sqr(cc[2]/ cc[4]) ;
          cr[2] := (cc[5] + 2 * cc[6]) * cc[10]  ;
            c := cc[9] * cc[8]*1000000.0 / (2 * cc[10])  ;
             ...
             w := cc[12];
             fdw := cr[3]/1.6;
if fdw > w*2 then showmessage (' **** Двигатель слишком мощный');
if fdw < w then
begin
showmessage ( '  ***** Мала мощность двигателя');
exit;
end;
...
a.caption:='Расчет студента '+fio_edit.Text;

//  WriteLn(Lst, '               Исходная информация :');
a.RichEdit1.Paragraph.Alignment:=paCenter;
a.RichEdit1.Lines.Add('Запорожский государственный технический университет');
a.RichEdit1.Lines.Add('Кафедра "ДМ и ПТМ"');
a.RichEdit1.Lines.Add('');
a.RichEdit1.Lines.Add('Динамика пуска цепного конвейера');
a.RichEdit1.Paragraph.Alignment:=paJustify;
a.RichEdit1.Lines.Add('');
a.RichEdit1.Lines.Add(chr(9)+'Расчет выполнил студент(ка)    '+fio_edit.Text);
a.RichEdit1.Lines.Add(chr(9)+'группы '+ Edit2.text);
a.RichEdit1.Lines.Add(chr(9)+'Дата  '+DateEdit1.text);
a.RichEdit1.Lines.Add('');
...
a.RichEdit1.Lines.Add(chr(9)+'Исходная информация :');
...
a.RichEdit1.Lines.Add(chr(9)+'14.Время разгона эл.двигателя'+chr(9)+floattostr(ct)+chr(9)+Label44.Caption);
//    ct:1:5,' c');
//   WriteLn(Lst,'  15.Время счета ........................  ',
a.RichEdit1.Lines.Add(chr(9)+'15.Время счета'+chr(9)+floattostr(cct)+chr(9)+Label45.Caption);
//    cct:2:4,' m/c');
{       Процедура вывода  расчетных данных на принтер.   }
//         WriteLn(Lst);
a.RichEdit1.Lines.Add('');
//         WriteLn(Lst,'        ***** Расчетные величины -' ) ;
a.RichEdit1.Lines.Add(chr(9)+'Расчетные величины');
//         WriteLn(Lst);
a.RichEdit1.Lines.Add('');
...
a.RichEdit1.Lines.Add('');
a.RichEdit1.Lines.Add('Расчет закончен');
end;
